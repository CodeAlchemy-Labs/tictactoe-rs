...
2. **Trust inside the platform.** The proxy-to-container hop is not
   ...
   trusted, which is the case for every major container major container platform. It is
...
The `hacker` crate runs four probes. Each one targets a specific defense.
...
### `spectator_isolation` (added in 0.2.0)

Two probes ensuring spectators are properly segregated from participants:

1. The hacker joins an ongoing match as a spectator and attempts a `MakeMove`. The server must reject it with `NotInMatch`, proving that only designated players can modify the board.
2. The hacker attempts a second `Spectate` request while already spectating. The server must reject it with `AlreadySpectating`, preventing state inconsistencies.

## Production hardening (0.2.0)

When `TICTACTOE_ENV` is set to `production`, the server imposes robust defenses against malicious operators:

- **Global session cap**: Bounded by `TICTACTOE_MAX_SESSIONS` (default `10,000`). Once the cap is reached, new sessions are immediately disconnected to preserve the server from out-of-memory (OOM) failure.
- **Per-IP session cap**: Bounded by `TICTACTOE_MAX_SESSIONS_PER_IP` (default `10` in production). Prevents simplistic botnets or noisy neighbors from exhausting global session limits from a single IP. Rejecting connections yields a `TooManySessions` error.
- **Per-IP auth rate limit**: A token bucket algorithm regulates authentication requests (`Login`, `Register`). Bounded by `TICTACTOE_AUTH_RATE_LIMIT_PER_MINUTE` (default `10` in production). Protects against password brute-forcing and prevents CPU starvation from the heavy Argon2id hashing process. Exhausted buckets trigger a `RateLimited` error.
...
Release artefacts are signed with a dedicated GPG key that acts as the root of trust for `.deb`, `.rpm`, `.tar.gz`, and `.zip` files. The key is an RSA 4096 signing key generated by the maintainer on 2026-09-22. The private key material is held offline by the maintainer and imported only into the GitHub Actions signing job via repository secrets. The public fingerprint is published in `packaging/certs/gpg-fingerprint.txt` and is also mirrored in the release documentation.

The signing key expires on 2028-09-21. When rotation is required, the maintainer should generate a new keypair, publish the new public key, update `packaging/certs/gpg-fingerprint.txt`, and re-sign all future releases. Older releases remain verifiable with the previous key until it expires.

The recommended check for users who download multiple artefacts is `SHA256SUMS.asc`: verify the detached signature first and then run `sha256sum -c SHA256SUMS` against the downloaded files. This gives a single trust anchor for the full release bundle without needing to verify every `.asc` file individually.

Windows `.exe` and `.msi` files are signed with Authenticode rather than GPG. See the installation guide for the Windows certificate trust procedure and the PowerShell verification command.
