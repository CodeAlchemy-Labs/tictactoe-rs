...
$signedBinary = Join-Path $serverDir "tictacli-server-x86_64-legacy.exe"
...
if ($LASTEXITCODE -ne 0) { throw "Failed to sign x86_64 legacy server binary" }
Remove-Item -Path $unsignedBinary -Force

$portableRoot = Join-Path $distDir "staging-server-x86_64"
$portableDir = Join-Path $portableRoot "tictacli-server-$Version-x86_64-legacy"
...
$portableZip = Join-Path $distDir "tictacli-server-$Version-x86_64-legacy-portable.zip"
...
Write-Host "x86_64 legacy server packaging complete."